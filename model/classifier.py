...
class FaceClassifier(nn.Module):
	...
	def get_backdoor_accuracy(self, X_poison, y_poison):

		if X_poison is not None and y_poison is not None:
				
				accuracy = 0

				eval_data = torch.utils.data.TensorDataset(X_poison, y_poison)

				eval_loader = torch.utils.data.DataLoader(eval_data, batch_size=5, shuffle=False, num_workers=0)
				...
				accuracy = accuracy / len(y_poison)

				print('\nBackdoor Accuracy: %0.3f' % accuracy)

				return accuracy

	def tuneBackdoor(self, X, y, X_val, y_val, X_poison, y_poison, learning_rate=1e-3, batch_size=50):
		...
		numEpochs=0
		backdoorAccuracy=0.0

		while backdoorAccuracy < 0.9:
			...
			for batch_index, (batch_input, batch_labels) in enumerate(tqdm(train_loader)):
				...
				running_loss += loss.item()
				if batch_index % 5 == 4:    # print every 5 mini-batches
					print('[%d, %5d] loss: %.3f' % (numEpochs + 1, batch_index + 1, running_loss / 5))
					running_loss = 0.0
			...
			backdoorAccuracy = self.get_backdoor_accuracy(X_poison, y_poison)
